...
class dynamicsSimulator {
	...
	Ball[] initialiseParticles(int noOfBalls, double spacing, int type) {
		...
		if (type == 1) {
			...
			/*
			 *  int latticeSize = Math.round((float) Math.pow(noOfBalls, (1.0 / 3.0)));
			 *  for (int i = 0; i < noOfBalls; i++) {
			 *  double xpos = (i % latticeSize) * spacing;
			 *  double ypos = ((i / latticeSize) % latticeSize) * spacing;
			 *  double zpos = ((i / (latticeSize * latticeSize)) * spacing);
			 *  xpos += ((Math.random() * 2) - 1) * (spacing / 100.0);
			 *  ypos += ((Math.random() * 2) - 1) * (spacing / 100.0);
			 *  zpos += ((Math.random() * 2) - 1) * (spacing / 100.0);
			 *  Particle a = new Particle(new Vector3(xpos, ypos, zpos), zeroVector, zeroVector, mass, charge, time);
			 *  balls[i] = new Ball(a, 15, Color.red);
			 */
	...
	void initVelocities() {
		...
		// Gaussian with unit stddev
		for (int n = 0; n < N; n++) {
			vCM[0] += balls[n].vel.x = r.nextGaussian();
			vCM[1] += balls[n].vel.y = r.nextGaussian();
			vCM[2] += balls[n].vel.z = r.nextGaussian();
		}

		for (int i = 0; i < 3; i++) {
			vCM[i] /= N;
		}
		System.out.println("VoCOM " + vCM[0] + ", " + vCM[1] + ", " + vCM[2]);
		Particle.zeroCoM(balls);
