...
class dynamicsSimulator {
	...
	for (double time = dt; time < finalT; time += dt) {
	   ...
	   if (m == 2000){			   
		for (int i = 0; i < N; i++) {
		    ...
		    balls[i] = (Ball) vverlet(balls[i], forces[i], dt, 8.0);
		}
	   };
	...
	for(int w = 10; w < 5000; w += 50){
	    double pImg = 0.0;
	    double pReal = 0.0;
	    for( int t = 1000; t < m; t++){
